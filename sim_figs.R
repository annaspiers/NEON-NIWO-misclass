...
sims_jm_plot %>%
  ...
  group_by(param, fraction, model) %>%
  summarize(capt_by95CI = sum(capt95)/n(),
            capt_by95CI_alt = sum(capt95)/length(capt95))
sims_jm_plot %>% filter(param=="Theta") %>%
  group_by(param, fraction, model) %>%
  summarize(capt_by95CI = sum(capt95)/n()) %>% head()
sims_jm_plot %>% filter(param=="psi") %>%
  mutate(rounded = round(fraction,digits=2) ) %>%
  group_by(rounded) %>%
  summarize(capt_by95CI = sum(capt95)/n()) %>%
  ggplot() +
  geom_point(aes(x=rounded,y=capt_by95CI))
sims_jm_plot %>%
  filter(param=="Theta") %>%
  group_by(param, fraction, model) %>%
  summarize(capt_by95CI = sum(capt95)/n()) %>%
...
stat_summary(aes(y =capt_by95CI,group=1), fun.y=mean, colour="red", geom="line",group=1)
...
sims_jm_plot %>%
  filter(param=="Theta") %>%
  head(1000) %>%
  ggplot(aes(x=fraction, y=abs(diff),col=model)) +
...
sims_jm_plot %>%
  ...
  geom_point(size=0.1,alpha=0.3) +
  geom_smooth() +
  facet_grid(rows=vars(param_model),scales="free") +
  labs(y="| Estimated Median - True Value |", x="Proportion of data verified")
ggsave("figures/simulations/dist_from_true_value_all.png")
...
sims_jm_plot %>%
  ...
  facet_grid(rows=vars(param_model),scales="free") +
  labs(x="Proportion of data labeled", y="95% CI width")
ggsave("figures/simulations/95CIwidth_all.png")

# #scrap
# scrap_psi <- sims_jm_plot %>% 
#     filter(param=="psi") %>% 
#     mutate(rounded = round(fraction,digits=2) ) %>% 
#     group_by(rounded) %>% 
#     summarize(mean=mean(ci95_width), 
#               sd=sd(ci95_width), 
#               med=median(ci95_width)) 
# scrap_psi %>%
#     ggplot() +
#     geom_point(aes(x=rounded,y=mean))
# ggsave("figures/simulations/95CIwidth_lambda.png")
...
sims_jm_plot %>%
    ...
    labs(x="Classification-only 95% CI width", y="Classification-occupancy 95% CI width",
...
sims_val %>%
    ...
    labs(x="Proportion of data verified", y="Validation Metric Value")
