...
all_paratax_df <- readRDS("occupancy/all_paratax_df.rds") %>%
  ...
  #AIS ^ has more pinned than sorted
pulled_inds <- readRDS("occupancy/pinned_df.rds") %>%
  filter(subsampleID == "aRw+8bWvW/wOIENap0etmXiCq6V1RZPfCkRcy1w8BLk=") %>%
  pull(individualID)
pinned_df <- readRDS("occupancy/pinned_df.rds") %>%
  filter(subsampleID != "aRw+8bWvW/wOIENap0etmXiCq6V1RZPfCkRcy1w8BLk=")
expert_df <- readRDS("occupancy/expert_df.rds") %>%
  filter(!(individualID %in% pulled_inds))
...
extra_colnames <- c(all_paratax_df %>%   #1) the unique parataxonomist and morphospecies ID's
                    ...
                    expert_df %>%   #2) the expert taxonomist IDs that the parataxonomist didn't use. 
                      distinct(scientificName) %>%
                      filter(!scientificName %in% unique(all_paratax_df$para_morph_combo)) %>% 
                      pull(scientificName) ) 
...
## Sanity check
assertthat::assert_that(sum(!is.na(para_new$exp_sciname)) == nrow(expert_df))
...
z.dat <- array(NA, dim = c(length(unique(para_new$plotID)),
                           length(rownames),
                           length(unique(all_paratax_df$col_year))),
               dimnames = list(sort(unique(para_new$plotID)), #plots
                               rownames, #expert IDs
                               unique(all_paratax_df$col_year)))
...
MCMCtrace(jm, params = paste0('growth\\[1,2\\]'), type = 'both', ind = F, pdf=F, ISB=F, Rhat=T) 
MCMCtrace(jm, params = paste0('growth\\[3,2\\]'), type = 'both', ind = F, pdf=F, ISB=F, Rhat=T) 
MCMCtrace(jm, params = paste0('growth\\[4,2\\]'), type = 'both', ind = F, pdf=F, ISB=F, Rhat=T) 
MCMCtrace(jm, params = paste0('growth\\[7,2\\]'), type = 'both', ind = F, pdf=F, ISB=F, Rhat=T) 
MCMCtrace(jm, params = paste0('growth\\[9,2\\]'), type = 'both', ind = F, pdf=F, ISB=F, Rhat=T) 
...
# MCMCtrace(jm, params = paste0('theta\\[5,5\\]'), type = 'both', ind = F, pdf=F, ISB=F, Rhat=T) 
